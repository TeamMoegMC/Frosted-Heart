    ...
    public static ActionResultType doProspect(PlayerEntity player,World world,BlockPos blockpos,ItemStack is,Hand h) {
        if (player != null && (!(player instanceof FakePlayer))) {//fake players does not deserve XD
            ...
            if (!world.isClientSide) {
                    ...
                    if (!founded.isEmpty()) {
                        ...
                        for (Entry<String, Integer> f : founded.entrySet()) {
                            if (rnd.nextInt((int) (f.getValue()*corr)) != 0) {
                                ...
                                s = s.append(GuiUtils.translateMessage("vein_size.count", rval).append(new TranslationTextComponent(f.getKey()).withStyle(TextFormatting.GREEN)).append(" "));
                                count++;
                            }
                        }
                        if (count > 0) {
                ...
                player.displayClientMessage(GuiUtils.translateMessage("vein_size.nothing").withStyle(TextFormatting.GRAY), false);
            }
        }
        return ActionResultType.SUCCESS;
    }
