        ...
        if (player != null && (!(player instanceof FakePlayer))) {//fake players does not deserve XD
            ...
            if (!world.isClientSide) {
                ...
                if (rnd.nextInt((int) (20*corr)) != 0) {//mistaken rate 5%
                    ...
                    if (!founded.isEmpty()) {
                        int count = 0;
                        MutableComponent s = GuiUtils.translateMessage("vein_size.found");
                        for (Entry<String, Integer> f : founded.entrySet()) {
                            if (rnd.nextInt((int) (f.getValue()*corr)) != 0) {
                                ...
                                s = s.append(GuiUtils.translateMessage("vein_size.count", rval).append(new TranslatableComponent(f.getKey()).withStyle(ChatFormatting.GREEN)).append(" "));
                                count++;
                            }
                        }
                        if (count > 0) {
                ...
                player.displayClientMessage(GuiUtils.translateMessage("vein_size.nothing").withStyle(ChatFormatting.GRAY), false);
            }
        }
        return InteractionResult.SUCCESS;
    }
