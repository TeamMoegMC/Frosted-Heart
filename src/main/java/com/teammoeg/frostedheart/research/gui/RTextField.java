...
public class RTextField extends Widget {
    ...
    public RTextField setText(Component txt) {
        ...
        if (maxLine > 0) {
            List<FormattedText> ls = theme.listFormattedStringToWidth(new TextComponent("").append(txt),
                    (int) (maxWidth / scale));
            formattedText = ls.subList(0, Math.min(ls.size(), (int) (maxLine / scale))).toArray(new FormattedText[0]);
        } else {
            formattedText = theme.listFormattedStringToWidth(new TextComponent("").append(txt), (int) (maxWidth / scale))
