...
public class RubbingTool extends FHBaseItem{
	...
	public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		if(hasResearch(stack)) {
			...
			if(rs!=null)
				tooltip.add(GuiUtils.translateTooltip("rubbing.current",rs.getName()).withStyle(TextFormatting.GOLD));
			else
				tooltip.add(GuiUtils.translateTooltip("rubbing.current.empty").withStyle(TextFormatting.GRAY));
		}else
			tooltip.add(GuiUtils.translateTooltip("rubbing.current.empty").withStyle(TextFormatting.GRAY));
		int points=getPoint(stack);
		if(points>0) {
			tooltip.add(GuiUtils.translateTooltip("rubbing.points",points));
			tooltip.add(GuiUtils.translateTooltip("rubbing.points.hint").withStyle(TextFormatting.YELLOW));
		}
