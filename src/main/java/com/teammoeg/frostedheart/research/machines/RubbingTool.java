...
public class RubbingTool extends FHBaseItem{
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		if(hasResearch(stack)) {
			...
			if(rs!=null)
				tooltip.add(GuiUtils.translateTooltip("rubbing.current",rs.getName()).withStyle(ChatFormatting.GOLD));
			else
				tooltip.add(GuiUtils.translateTooltip("rubbing.current.empty").withStyle(ChatFormatting.GRAY));
		}else
			tooltip.add(GuiUtils.translateTooltip("rubbing.current.empty").withStyle(ChatFormatting.GRAY));
		int points=getPoint(stack);
		if(points>0) {
			tooltip.add(GuiUtils.translateTooltip("rubbing.points",points));
			tooltip.add(GuiUtils.translateTooltip("rubbing.points.hint").withStyle(ChatFormatting.YELLOW));
		}
